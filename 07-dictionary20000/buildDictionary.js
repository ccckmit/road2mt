...
KB.set=function(map, key, value, replace=false) { // , replace=false
  if (typeof map[key] !== 'undefined' && value !== map[key]) {
    if (value.endsWith("s") || value.endsWith("ed") || value.endsWith("ing")) {
      nouseDic.push("");
      nouseDic.push("// "+map[key]+'='+key)
      nouseDic.push(value+'='+key);
      return false; 
    }
    console.log("");
...
KB.dic2json = function(s2t, t2s, dic, replace=false) {
  ...
  for (var i=0, len=lines.length; i<len; i++) {
    ...
    if (key.indexOf("'")<0) {
      KB.set(s2t, key, value, replace);
      KB.set(t2s, value, key, replace);
      filterDic.push(line);
    }
  }
}
